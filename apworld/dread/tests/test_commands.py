...
def test_help_mentions_bridge_commands():
    r = parse_command("help")
    assert r.info
    assert "/bridge_status" in r.info
    assert "/switches" in r.info
    assert "/promote_switch" in r.info


def test_help_does_not_mention_removed_commands():
    """The old /dread_connect, /switch_host, /switch_reconnect commands are
    gone — the Switch dials in, no IP entry needed."""
    r = parse_command("help")
    assert r.info
    assert "/dread_connect" not in r.info
    assert "/switch_host" not in r.info
    assert "/switch_reconnect" not in r.info
