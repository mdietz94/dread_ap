...
Pages (sequenced; each calls `next_page()` when its work completes):
  ...
  3. DumpPickerPage    — file dialog for the user's Dread 2.1.0 NSP or XCI
  ...
  7. DonePage          — "Launch DreadClient" button (if a .dreadap was passed)
...
from .dreadap_file import DreadapFile, parse_dreadap, dreadap_to_launch_args
...
def run_setup_wizard(dreadap_path: str | None = None) -> bool:
    ...
    `dreadap_path` is the .dreadap file the user opened (if any) — used to
    pre-fill DreadClient on the "Launch now" button at the end. Pass None
    ...
    wizard_state: dict[str, Any] = {
        ...
        # returns and performs the actual DreadClient launch.
    ...
    def build_welcome() -> Screen:
        ...
        msg = (
            "This wizard prepares everything DreadClient needs to talk to a "
            ...
            "  - Dread version 1.0.0. If you're on 1.1.0+, downgrade first "
            ...
            "  - Extract moon + capture name tables from your own Dread 2.1.0 "
            ...
            "— those go through DreadClient's Connect bar."
        )
    ...
    def build_nsp() -> Screen:
        s = Screen(name="nsp")
        root = BoxLayout(orientation="vertical", padding=20, spacing=12)
        root.add_widget(_h1("Pick your Dread 2.1.0 dump"))
        root.add_widget(_label(
            ...
            "extracted to %APPDATA%/dread_ap/data/ and never leave "
            "your machine.\n\n"
            "XCI dumps additionally need your title.keys (alongside "
            "prod.keys) to contain the Dread entry — NSPs ship their own "
    ...
    def build_extract() -> Screen:
        ...
        def run_in_worker() -> None:
            ...
            # every legitimate Dread 2.1.0 source (eShop NSP, cartridge,
            ...
            if outputs_present:
                ...
                if checks and all(c.match for c in checks):
                    ...
                    if mismatched and not hash_hint:
                        hash_hint = (
                            "Maps don't match the canonical Dread 2.1.0 "
                            "USen fingerprint. Confirm your dump is "
                            "Dread 2.1.0 (not 1.1.0+ or a different game), "
        ...
        def start_worker() -> None:
            ...
            if _state["attempt_count"] >= MAX_STEP_ATTEMPTS:
                msg = (
                    ...
                    f"wrong Dread version (need 1.0.0), corrupt NSP/XCI "
        ...
        def _populate_inner() -> None:
            ...
            if result:
                summary = (
                    ...
                    "card, plug it into the Switch, boot Dread. The mod loads "
                    "automatically.\n"
                    "  - For Ryujinx: boot Dread in Ryujinx; the mod is "
                    "already in the mods directory.\n"
                    "  - For Custom folder: the files are laid out under "
                    "`atmosphere/contents/010093801237c000/{exefs,romfs}/` "
                    ...
                    "(DreadClient and the Switch mod ship in lockstep; "
                    "DreadClient will refuse to connect on a version "
                    ...
                    "the Connect bar in DreadClient."
                )
