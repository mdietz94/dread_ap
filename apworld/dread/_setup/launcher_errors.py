...
(double-click a `.dreadap`, or right-click → Open With → ArchipelagoLauncher),
...
`_setup.wizard` or a missing `.dreadap` file just looks like "nothing happened"
to the user.

This module exports a `_visible_errors(context)` decorator that wraps the
apworld's subprocess entry points so any escape:
  1. lands a full traceback at `%APPDATA%/dread_ap/launch-crash.log`
...
def show_launch_error(
    ...
    *,
    ...
    log_path_msg = ""
    try:
        written_to = log_writer(f"=== Dread Archipelago launch crash ({context}) ===\n{tb_text}")
        if written_to:
    ...
    try:
        notifier(
            f"Dread Archipelago — {context} failed",
...
def _default_log_writer(text: str) -> str | None:
    """Write the crash log under %APPDATA%/dread_ap/. Returns the
