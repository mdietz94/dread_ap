...
purely so a future non-star map would Just Work; today every entry is Trivial
or a per-region E-Tank floor. If no compiled rules are present (pre-bake dev
env), regions fall back to trivial connections too, so the apworld still loads.
"""
...
def create_regions(world) -> None:
    ...
    for name in region_names:
        ...
        if rule_ast == TRIVIAL_RULE:
            ...
            menu.connect(region, name,
                         rule=compile_to_lambda(rule_ast, player, trick_levels))
