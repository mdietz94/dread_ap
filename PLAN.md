...
3. **Cutscene-blocked item delivery.** Randovania's `RL.GivePendingPickup` documents a retry loop because `Scenario.QueueAsyncPopup` no-ops during cinematics / user-input-disabled states. AP items received during a cutscene queue but don't deliver until the player regains control. **Mitigation**: same as Randovania — implement the pending-queue + post-HELLO replay pattern (the smo_archipelago client already has this pattern in `state.py`; lift it).
   > **Correction (post-implementation).** The "just lift it" mitigation is NOT safe as written. Our client calls `RandomizerPowerup.OnPickedUp` directly (current exlaunch doesn't define `RL.ReceivePickup`), and that path is non-idempotent — `inventory_index` is a no-op and `PACKET_RECEIVED_PICKUPS` is ignored — so a naive replay double-grants additive items (Missile/Energy/Power Bomb tanks) on reconnect. The replay must be preceded by idempotent, `ReceivedPickups`-gated delivery, which itself needs hardware-validated counter semantics. See CLAUDE.md "Known unknowns / risks #1" for the current authoritative writeup.
